...
# FIX SEGURIDAD: Contraseña desde st.secrets, con fallback para desarrollo local
try:
    PASSWORD_ADMIN = st.secrets["PASSWORD_ADMIN"]
except Exception:
    PASSWORD_ADMIN = "GorrasCba2026"  # Solo para desarrollo local
...
# FIX IDs DUPLICADOS: Generar el próximo ID de pedido de forma segura
def generar_id_pedido(df_pedidos):
    if df_pedidos.empty:
        return "P0001"
    nums = df_pedidos['ID_Pedido'].str.extract(r'P(\d+)')[0].dropna().astype(int)
    if nums.empty:
        return "P0001"
    return f"P{nums.max() + 1:04d}"
...
if opcion == "🛍️ Catálogo de Clientes":
    ...
    if df_disponible.empty:
        ...
        if not pedido_cliente:
            ...
            # FIX RESET ESTADO: Si el carrito se vacía manualmente, resetear el estado
            ...
            if total_items < MINIMO_UNIDADES:
                ...
                # FIX NAMEEERROR: Los inputs se definen ANTES del if/else para que estén
                # disponibles tanto al reservar como al mostrar el PDF y el link de WhatsApp
                nombre_cliente = st.sidebar.text_input(
                    "Tu Nombre:",
                    disabled=st.session_state.get('pedido_procesado', False)
                ).strip()
                localidad_cliente = st.sidebar.text_input(
                    "Ciudad / Provincia:",
                    disabled=st.session_state.get('pedido_procesado', False)
                ).strip()
                ...
                if nombre_cliente and localidad_cliente:
                    if not st.session_state.get('pedido_procesado', False):
                        if st.sidebar.button("🔒 1. Reservar Pedido", use_container_width=True):
                            ...
                            # VALIDACIÓN ANTINEGATIVOS: Verificar stock real antes de tocar nada
                            hay_stock_suficiente = True
                            mensajes_error = []
 
                            for p_id, info in pedido_cliente.items():
                                fila_producto = df_stock_actual[df_stock_actual['ID'] == p_id]
                                if not fila_producto.empty:
                                    stock_real = int(fila_producto['Stock'].values[0])
                                    if stock_real < info['Cantidad']:
                                        hay_stock_suficiente = False
                                        mensajes_error.append(f"• **{info['Modelo']}**: Pediste {info['Cantidad']} u. y solo quedan {stock_real} u.")
                                else:
                                    hay_stock_suficiente = False
                                    mensajes_error.append(f"• El producto **{info['Modelo']}** ya no existe en el sistema.")
 
                            if not hay_stock_suficiente:
                                st.sidebar.error("⚠️ ¡Stock insuficiente! Alguien compró mientras armabas tu carrito:")
                                for err in mensajes_error:
                                    st.sidebar.write(err)
                                st.sidebar.info("Por favor, modificá las cantidades en el catálogo y volvé a intentar.")
                            else:
                                for p_id, info in pedido_cliente.items():
                                ...
                                # FIX IDs DUPLICADOS: usar la función segura
                                nuevo_id = generar_id_pedido(df_pedidos_actual)
                        ...
                        # FIX ENCODING: bytes() es compatible con fpdf2 y soporta tildes y ñ
